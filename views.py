...
try:
        x = float(xx.output2[row])
except:
        responses.append(xx.output2[row])


try:
        x = float(xx.output3[row])
except:
        responses.append(xx.output3[row])



for row in range (0,len(xx)):
        responses = []
        responses.append(xx.output1[row])
        try:
                x = float(xx.output2[row])
        except:
                responses.append(xx.output2[row])
        try:
                ...
                responses.append(xx.output3[row])
        pairs.append([(xx.input[row]), responses])
...
def chatbot(request):
    if request.method == 'GET':
        # engine = pyttsx3.init()
        # engine.say("Welcome to chatbot. Howcan I help you ?")
        # engine.runAndWait()
        # time.sleep(0.01)
        # engine = pyttsx3.init()
        f = open("chatting.txt", "w")
        f.write('0')
        f.close()
        ...
        try:
            ...
            f = open("chatting.txt", "r")
            chat_mode = int(f.read())
            if chat_mode == 1:
                reply = "Disease discussion"
                return render(request,'hospital/chatbot.html',{'symptoms':symptoms})                    
            else:
                chat = Chat(pairs, reflections)
                reply = chat.respond(question)
                # engine = pyttsx3.init()
                # engine.say(reply)
                # engine.runAndWait()
                # time.sleep(0.01)
                # engine.stop()
                print(reply)
            return render(request,'hospital/chatbot.html',{'reply':reply,'question':question})
            ...
            selected_symptom = request.POST['selected_symptom']            
            ...
            return render(request,'hospital/chatbot.html',{'disease_name':disease_name,'description_of_disease':description_of_disease,'precutions_measures':precutions_measures})
